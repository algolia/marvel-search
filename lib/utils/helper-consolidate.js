...
const Helper = {
  ...
  merge(data) {
    ...
    _.each(commonKeysDbpediaInfobox, (key) => {
      ...
      let chosenValue = dbpediaValue || infoboxValue;
      // Take default value from dbpedia if a string
      if (_.isString(chosenValue)) {
        wikipediaData[key] = chosenValue;
        return;
      }
      // Merge arrays
      if (_.isArray(chosenValue)) {
        wikipediaData[key] = _.uniq(_.flatten(_.compact([dbpediaValue, infoboxValue])));
        return;
      }
    });

    let name = Helper.getName(data, wikipediaData);
  ...
  getName(originalData, wikipediaMergedData) {
    return _.get(originalData, 'marvelApiData.name') || wikipediaMergedData.name;
  },

  getImages(data) {
    // First get the image from Marvel website, and fallback to Marvel API
    // then wikipedia
    let thumbnail = _.get(data, 'marvelWebsiteData.thumbnail') ||
                    _.get(data, 'marvelApiData.image') ||
