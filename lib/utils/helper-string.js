...
const Helper = {
  ...
  rejectBadItemsInList(data) {
    ...
    return _.reject(data, (item) => {
      ...
      // "As XXX"
      if (_.startsWith(item.toLowerCase(), 'as ')) {
        return true;
      }
      // "Ability to XXX"
      if (_.startsWith(item.toLowerCase(), 'ability to ')) {
        return true;
      }
      // "Transform into XXX"
      if (_.startsWith(item.toLowerCase(), 'transform into ')) {
        return true;
      }
      // "-sense"
      ...
      // Starts with "s "
      if (_.startsWith(item.toLowerCase(), "'s")) {
        return true;
      }
      // Starts with "vol. XX"
      if (_.startsWith(item.toLowerCase(), 'vol. ')) {
        return true;
      }
