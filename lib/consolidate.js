 ...
 * records
 **/
// Pour chaque record, trouver le nom principal, et le name normal. Le nom
// principale est le nom du personnage quand plusieurs identités
//
// Pour chaque record, l'associer à un perso marvel quand le nom est facile (ie,
// sans parentheses), le même que le nom master
//
// pour chaque field, selon que ce soit une identité ou un perso simple, on
// change la liste des fallbacks
//
...
function cleanUpBadWikiPages(characters) {
  return _.filter(characters, (character, pageName) => {
    console.info(pageName, character);
  ...
  return HelperPath
    ...
    .then((characters) => {
      return _.reject(characters, (character) => {
        // LEGO heroes
        if (character.name.match(/LEGO Marvel/)) {
          return true;
        }
        // Ultimate characters
        if (character.name.match(/\(Ultimate\)/)) {
          return true;
        }
        // 2099 characters
        if (character.name.match(/\(2099\)/)) {
          return true;
        }
        return false;
      });
...
function consolidate(characters) {
  ...
  return _.reject(mergedCharacters, (character) => {
