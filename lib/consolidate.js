...
function initCharacters() {
  return helper.readJSON(urlsFile).then((urls) => {
    let characters = {};
    _.each(urls, (url) => {
      let name = helper.getWikipediaPageName(url);
      characters[name] = {
        wikipediaUrl: url
      };
    });
...
function addPageviews(characters) {
  let allPromises = _.map(characters, (character, pageName) => {
    ...
    return helper.readJSON(path).then((pageviewData) => {
...
function addDBpedia(characters) {
  let allPromises = _.map(characters, (character, pageName) => {
    ...
    return helper.readJSON(path).then((rawData) => {
      if (!rawData) {
        return null;
      }
      // We first get a more readable dump of the DBPedia data, with less nested
      // levels
      let simplifiedData = helper.simplifyDBPedia(pageName, rawData.dbpediaData);
      ...
      let dbpediaData = {
        ...
        aliases: _.get(simplifiedData, 'property.aliases'),
        alliances: _.get(simplifiedData, 'property.alliances'),
        authors: _.get(simplifiedData, 'property.creators'),
        powers: _.get(simplifiedData, 'property.powers'),
        specy: _.get(simplifiedData, 'property.species')
      };

      // We clean up the content to remove any mention of the DBPedia
      dbpediaData = helper.cleanDBPedia(dbpediaData);
...
function addWikidata(characters) {
  let allPromises = _.map(characters, (character, pageName) => {
    ...
    return helper.readJSON(path).then((wikidataData) => {
      if (!wikidataData) {
        return null;
      }
      // Update characters in place
